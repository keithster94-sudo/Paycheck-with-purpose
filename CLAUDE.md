...
- **Pages** (`src/pages/`): `Dashboard` (zero-based budgeting summary + per-purpose progress bars), `Paychecks`, `Purposes` (create/manage envelopes), `Transactions` (log spend against a purpose), `Reports` (archive a month across every purpose into a client-reviewable report of spending plus goal progress; see below).
...
- Transactions are never deleted when a month rolls over — `resetsMonthly` only changes what's *displayed* (via the date filtering above), so a purpose's full history always remains in `transactions`. `useBudgetStore().archiveMonth(period)` (Reports.tsx) generates an `ArchivedReport` for a "YYYY-MM" period by filtering that same `transactions` array, one `ArchivedCategoryReport` per purpose that existed at archive time (all purposes, not just `resetsMonthly` ones), snapshotting each category's name/allocated amount/goal plus its transactions dated within that month. Archiving the same period again replaces (upserts by `period`) rather than duplicates.
  - Each `ArchivedCategoryReport.balance` (`allocated` minus spending summed through the end of that archived month, i.e. `t.date.slice(0,7) <= period`) is a point-in-time snapshot, not "the current live balance" — this is what makes archiving several consecutive months produce a real month-over-month progress trail for `goal`-bearing purposes (see the goal progress bar in `Reports.tsx`) instead of repeating today's number in every report.
